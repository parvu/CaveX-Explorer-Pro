...
### Phase 3: x500 air-pocket SLAM (planned, this branch)
...
air pocket above. Not implemented yet on this branch -- x500 has no
independent flight controller, SLAM node, or launch entry of its own
today; this section is the placeholder for that work (PX4 SITL or a
direct gz-transport thrust controller matching Phase 2's own
`ApplyLinkWrench` pattern, plus a lidar/camera-based SLAM node reusing
`cavex_gtsam_slam`'s scan-registration code the way Phase 2 does).
