...
Only actually publishes thrust while "manual_on" is in effect (toggled by
the Manual button) -- while off, this node does nothing, so an autonomous
script (ate_circle_demo.py etc.) started separately keeps sole control of
the same thruster topics without the two fighting over them. Running both
manual mode ON and an autonomous script at the same time is not guarded
against -- same as this project's other stand-alone demo scripts, that's
on the operator, not a case this node resolves for you.
...
def main():
    ...
    try:
        while True:
            fx_b, fy_b, fz_b = COMMANDS[_state["cmd"]] if _state["manual_on"] else (0.0, 0.0, 0.0)
