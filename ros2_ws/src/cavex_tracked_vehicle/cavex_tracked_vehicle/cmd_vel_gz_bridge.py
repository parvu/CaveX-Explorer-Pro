...
This node gives autonomous driving the same bypass, body-frame passthrough
(no world-frame conversion needed -- unlike cmd_vel_to_ardupilot.py's AHRS
...
Real bug found live 2026-08-27: briefly split tracks and props onto
separate gz-transport topics (an SDF <topic> override on the TrackedVehicle
plugin, real attempt at mutual exclusion via vehicle_switch_node.py's
locomotion-mode state machine) -- reverted, see manual_gui_bridge.py's own
comment for the full story (<topic> silently breaks that plugin's Twist
subscription; confirmed live). Back to the single shared cmd_vel topic;
boat_thruster_control.py still gates its own thrust output off
/cavex/locomotion_mode independently.
"""
import rclpy
